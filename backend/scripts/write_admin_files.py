...
BASE_CTRL = r'c:\Users\Dhiren\Downloads\New Shubh Milan App\New Shubh Milan App\backend\src\controllers\admin'
BASE_ROUTES = r'c:\Users\Dhiren\Downloads\New Shubh Milan App\New Shubh Milan App\backend\src\routes\admin'

os.makedirs(BASE_CTRL, exist_ok=True)
os.makedirs(BASE_ROUTES, exist_ok=True)

# ─── admin.users.controller.ts ────────────────────────────────────────────────
ADMIN_USERS_CTRL = r'''import { Request, Response } from 'express';
...
export async function updateUser(req: Request, res: Response): Promise<void> {
  try {
    ...
    const db = getDB(); const users = db.users as UserRow[];
...
export async function adjustCredits(req: Request, res: Response): Promise<void> {
  try {
    ...
    const db = getDB(); const credits = db.credits as CreditRow[]; const history = db.credits_history as CreditHistoryRow[];
...
export async function approveAllDocuments(req: Request, res: Response): Promise<void> {
  try {
    const { userId } = req.params; const adminId = req.user!.id;
    const db = getDB(); const docs = db.documents as DocumentRow[]; const now = new Date().toISOString(); let count = 0;
...
export async function getUserChats(req: Request, res: Response): Promise<void> {
  try {
    const { userId } = req.params; const db = getDB();
...
export async function deleteUserDocument(req: Request, res: Response): Promise<void> {
  try {
    const { userId, docType } = req.params; const db = getDB();
    const docs = db.documents as DocumentRow[];
...
with open(os.path.join(BASE_CTRL, 'admin.users.controller.ts'), 'w', encoding='utf-8') as f:
    f.write(ADMIN_USERS_CTRL)
print("admin.users.controller.ts written:", len(ADMIN_USERS_CTRL), "chars")
